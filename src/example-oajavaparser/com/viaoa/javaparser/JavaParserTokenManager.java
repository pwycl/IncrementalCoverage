...
public class JavaParserTokenManager implements JavaParserConstants {
	...
	public JavaParserTokenManager(JavaParser parserArg, JavaCharStream stream) {
		parser = parserArg;
	...
	public JavaParserTokenManager(JavaParser parserArg, JavaCharStream stream, int lexState) {
		...
		EOFLoop: for (;;) {
			...
			for (;;) {
				...
				if (jjmatchedKind != 0x7fffffff) {
					...
					//qqqqqqqqqqq needs to be true qqqqqqqqqqqqqqqqqqqq
					if (jjmatchedKind == ARROW || ((jjtoToken[jjmatchedKind >> 6] & (1L << (jjmatchedKind & 077))) != 0L)) {
						matchedToken = jjFillToken();
						matchedToken.specialToken = specialToken;

...
/**
 * Grammar to parse Java version 1.5 <br>
 * NOTE !!!!!!! this has been customized for Java > 1.5 and should not be overwritten by the javacc code generator
 *
 * @author Sreenivasa Viswanadha - Simplified and enhanced for 1.5
 */

public class JavaParser/*@bgen(jjtree)*/ implements JavaParserTreeConstants, JavaParserConstants {/*@bgen(jjtree)*/
	public JJTJavaParserState jjtree = new JJTJavaParserState();
	...
	// these methods are added to JavaParser.java since "NODE_SCOPE_HOOK = true"
	void jjtreeOpenNodeScope(Node n) {
		if (((SimpleNode) n).beginToken == null) {
			((SimpleNode) n).beginToken = token;
		}
	}

	void jjtreeCloseNodeScope(Node n) {
		if (((SimpleNode) n).endToken == null) {
			((SimpleNode) n).endToken = token;
		}
	}
	...
	final public void TypeArguments(final boolean bRequired) throws ParseException {
		...
		try {
			...
			//qqqqqqqq
			if (bRequired || ((jj_ntk == -1) ? jj_ntk() : jj_ntk) != GT) {
	...
	final public void Function() throws ParseException {
		ASTExpression jjtn000 = new ASTExpression(this, JJTFUNCTION);
		boolean jjtc000 = true;
		jjtree.openNodeScope(jjtn000);
		jjtreeOpenNodeScope(jjtn000);
		try {
			Expression();
			//qqqqqqqqq check for Arrow
			//qqqqqqqqqqqq
			if (jj_ntk == ARROW) {
				// qqq block for statement
				jj_consume_token(ARROW);

				jj_ntk();
				if (token.next != null && token.next.kind == LBRACE) {
					Block();
				} else {
					Expression();
				}
	...
	public JavaParser(java.io.InputStream stream, String encoding) {
		...
		token_source = new JavaParserTokenManager(this, jj_input_stream);
		token = new Token();
		jj_ntk = -1;
	}

...
final class LineBuffer {
    ...
    int read() throws IOException{
        ...
        if (bufferBuilder == null){
            bufferBuilder = new StringBuilder(String.valueOf(buffer));
        }

        return res;
    }

    int read_old() throws IOException {
        if (end == offset && !fill()){
    ...
    String subString(int src){
        return bufferBuilder.substring(src);
    }

    String subString(int src, int end){
        return bufferBuilder.substring(src,end);
//        String res = new String(buffer, src,
//                end - src);
//        return res;
    }
...
//        e.printStackTrace();
        /**
         * introduce symbolic values (zhenbang)
         */
//        for (int i = 0; i < end; i++) {
//        	char c = Debug.makeConcolicChar("sym_char_" + sym_index, ""+(int)buffer[i]);
//        	System.out.println("------" + sym_index+"--"+ c + ":"+(int)c);
//        	sym_index ++;
//        	buffer[i] = c;
//        }

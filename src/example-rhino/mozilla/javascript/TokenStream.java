...
//         TSF_LOOKAHEAD   = 0x0100,  // looking ahead for a token
     ...
     * @throws IOException
     */
    public static int sym_index = 0;
    public static int oldtoken = -1;
    public static int oldbp = -1;

    public static int srcIdx = 0;
    public static String tokenImage = "";
    public int getToken() throws IOException {
    	int r = getToken(0);
//        int offset = in.getOffset();
        int offset = in.getFieldOffset();
        if (srcIdx == offset) {
            tokenImage = in.subString(srcIdx, offset+1);
        }
        else if (srcIdx < offset){
            tokenImage = in.subString(srcIdx, offset);
            srcIdx = offset;
        }
        else if (srcIdx > offset){
            tokenImage = in.subString(srcIdx);
            srcIdx = offset = in.getEndPos();
        }

        if (oldtoken == r && oldbp == offset) {
            return r;
        }else {
            oldtoken = r;
            oldbp = offset;
        ...
        System.out.println("------sym_token---" + (tokenno-1)+"--:"+ r);
        if (!tokenImage.trim().isEmpty())
            System.out.println("Token: " + r +", offset: " + offset + ", tokenImage: " + tokenImage);
    	return r;
    	/*added by zhenbang for introducing symbolic variables*/
//    	if (r > 0) {
//    		int result = Debug.makeConcolicInteger("sym_token_" + (tokenno-1), ""+r);
// //   		System.out.println("------sym_token---" + (tokenno-1)+"--:"+ result);
//    		return result;
//    	}
//    	//sym_index++;
//    	return r;
    ...
    public int getToken(int a) throws IOException {
        int c;
        tokenno++;
